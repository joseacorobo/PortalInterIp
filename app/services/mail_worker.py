...
class MailWorker:
    ...
    def _sync_simulator(self, area: str = None) -> dict:
        """
        Modo simulador de laboratorio: genera casos telco estructurados
        para las 6 áreas técnicas oficiales aplicando el modelo P1 a P5.
        """
        sample_cases = [
            # 1. ACCESO Y APROVISIONAMIENTO (P3 - 3 pts)
            {
            ...
            # 2. ACCESO Y APROVISIONAMIENTO (P4 - 5 pts)
            {
            ...
            # 3. ACCESO Y APROVISIONAMIENTO (P2 - 2 pts)
            {
                "sender": "cuadrilla.valencia@inter.com.ve",
                "subject": "Discrepancia MAC en BD 815 / MAC Roja",
                "body": f"Buenas tardes, cliente AB {random.choice(['10', '25', '30'])}{random.randint(10000000, 99999999)} con ONT FHTT{random.randint(10000000, 99999999)} en OLT-VAL-01 presenta mac en rojo en el 815. Favor corregir aprovisionamiento.",
                "area": "Soporte",
                "depto": "ACCESO_APROV"
            },
            # 4. REDES WAN (P5 - 8 pts)
            {
            ...
            # 5. TELEFONÍA (P3 - 3 pts)
            {
                "sender": "soporte.voip@inter.com.ve",
                "subject": "Falla registro SIP en ONT residencial",
                "body": f"AB {random.choice(['10', '25', '30'])}{random.randint(10000000, 99999999)} con serial FHTT{random.randint(10000000, 99999999)} en OLT-VAL-02 reporta SIP 403 Forbidden. Revisar dialplan y auth.",
                "area": "Telefonía",
                "depto": "TELEFONIA"
            },
            # 6. SEGURIDAD (P4 - 5 pts)
            {
                "sender": "soc.seguridad@inter.com.ve",
                "subject": "Alerta Firewall Perimetral: Tráfico anómalo DDoS",
                "body": f"Detección de ataque volumétrico hacia bloque IP de abonado {random.choice(['10', '25', '30'])}{random.randint(10000000, 99999999)}. Se requiere aplicar políticas perimetrales de mitigación DDoS en firewall.",
                "area": "Seguridad",
                "depto": "SEGURIDAD"
            },
            # 7. CONTROL DE TRÁFICO E INALÁMBRICAS (P3 - 3 pts)
            {
                "sender": "noc.wireless@inter.com.ve",
                "subject": "Degradación de radioenlace microondas / Ajuste QoS",
                "body": "Radioenlace microondas troncal PTP presenta saturacion rf al 84% con pérdida de paquetes. Se requiere ajuste de colas QoS y shaping en router wireless.",
                "area": "Control de Tráfico",
                "depto": "TRAFICO_INALAMBRICO"
            },
            # 8. GRANDES CLIENTES (P4 - 5 pts)
            {
                "sender": "cuentas.vip@inter.com.ve",
                "subject": "Requerimiento VIP Cliente Corporativo Banco Mercantil",
                "body": f"Cliente corporativo con AB {random.choice(['10', '25', '30'])}{random.randint(10000000, 99999999)} solicita atención VIP por enlace dedicado simétrico de 100 Mbps con degradación de throughput.",
                "area": "Grandes Clientes",
                "depto": "GRANDES_CLIENTES"
            }
        ]
        
        candidates = sample_cases
        if area and area not in ["Todas", "General", ""]:
            filtered = [c for c in sample_cases if c.get("area", "").lower() == area.lower() or c.get("depto", "").lower() == area.lower()]
            if filtered:
                candidates = filtered
                
        selected = random.choice(candidates)
        msg_id = f"<sim-{int(time.time()*1000)}-{random.randint(10000,99999)}@inter.com.ve>"
    ...
    def _insert_ticket(
        ...
        """
        Inserta de forma atómica y segura un nuevo ticket en SQLite (ip_ops.db).
        - Transacción protegida con try / except / finally y conn.rollback().
        - Deduplicación estricta por Message-ID, ventana temporal y código único.
        - Extracción de entidades técnicas con TelcoEmailParser.
        - Inyección del modelo de puntos por complejidad DERS (P1 a P5).
        - Asignación al departamento oficial y registro inicial en ticket_historial_estados.
        """
        ...
        try:
            ...
            # -------------------------------------------------------------
            # 1. DEDUPLICACIÓN: Verificar Message-ID (RFC 2822 / RFC 3501)
            # -------------------------------------------------------------
            if message_id:
                cur.execute("SELECT id, ticket_code FROM email_tickets WHERE message_id = ?", (message_id,))
                dup_msg = cur.fetchone()
                if dup_msg:
                    logger.info(f"Deduplicación activa: Ticket ya existente con Message-ID {message_id} ({dup_msg['ticket_code']})")
                    return None

            # -------------------------------------------------------------
            # 2. DEDUPLICACIÓN: Ventana temporal por remitente y asunto (10 min)
            # -------------------------------------------------------------
            from datetime import timedelta
            recent_cutoff = (datetime.now() - timedelta(minutes=10)).strftime("%Y-%m-%d %H:%M:%S")
            cur.execute("""
            SELECT id, ticket_code FROM email_tickets 
            WHERE sender_email = ? AND subject = ? 
              AND COALESCE(fecha_creacion, created_at) >= ?
            """, (sender_email, subject, recent_cutoff))
            dup_content = cur.fetchone()
            if dup_content:
                logger.info(f"Deduplicación temporal: Correo idéntico recibido recientemente de {sender_email}: '{subject}' ({dup_content['ticket_code']})")
                return None

            # -------------------------------------------------------------
            # 3. EXTRACCIÓN TÉCNICA CON TelcoEmailParser
            # -------------------------------------------------------------
            full_content = f"{subject}\n{body_text}"
            p = TelcoEmailParser.parse(full_content)

            # -------------------------------------------------------------
            # 4. RESOLUCIÓN DE DEPARTAMENTO TÉCNICO OFICIAL
            # -------------------------------------------------------------
            depto_code = p.get("departamento_codigo", "ACCESO_APROV")
            cur.execute("SELECT id, nombre FROM departamentos WHERE codigo = ?", (depto_code,))
            row_d = cur.fetchone()
            if row_d:
                departamento_id = row_d["id"]
                departamento_nombre = row_d["nombre"]
            else:
                departamento_id = 1
                departamento_nombre = "Redes de Acceso y Aprovisionamiento"

            # -------------------------------------------------------------
            # 5. MODELO DE PUNTOS POR COMPLEJIDAD (P1-P5) - docs/feature_control_correos.md
            # -------------------------------------------------------------
            task_code = p.get("suggested_task_code", "P1-SOP-01")
            cur.execute("SELECT id, points, name, sla_minutes FROM task_types WHERE code = ?", (task_code,))
            row_t = cur.fetchone()
            if row_t:
                task_type_id = row_t["id"]
                task_points = row_t["points"]
                task_name = row_t["name"]
            else:
                # Si la tarea no existe en catálogo, registrarla al vuelo garantizando la ponderación P1-P5
                task_name = p.get("suggested_task_name", "Operación Estándar")
                task_points = p.get("suggested_points", 1)
                cur.execute("""
                INSERT INTO task_types (code, name, area, points, sla_minutes, description)
                VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    task_code,
                    task_name,
                    p.get("detected_area", "Soporte"),
                    task_points,
                    30,
                    f"Catálogo DERS autogenerado para {task_code}"
                ))
                task_type_id = cur.lastrowid

            # -------------------------------------------------------------
            # 6. GENERACIÓN DE CÓDIGO ÚNICO DE TICKET (INC-XXXXX)
            # -------------------------------------------------------------
            code = forced_code
            if code:
                cur.execute("SELECT id FROM email_tickets WHERE ticket_code = ?", (code,))
                if cur.fetchone():
                    code = None
            if not code:
                for _ in range(15):
                    candidate = f"INC-{random.randint(10000, 99999)}"
                    cur.execute("SELECT id FROM email_tickets WHERE ticket_code = ?", (candidate,))
                    if not cur.fetchone():
                        code = candidate
                        break
                if not code:
                    code = f"INC-{int(time.time() * 1000) % 100000}"

            now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # -------------------------------------------------------------
            # 7. INSERCIÓN ATÓMICA DEL TICKET EN ESTADO 'PENDIENTE'
            # -------------------------------------------------------------
            cur.execute("""
            INSERT INTO email_tickets (
                ticket_code, sender_email, recipient_email, subject, full_body, html_body,
                area, departamento_id, subscriber_code, serial_pon, node_name, slot_pon,
                mac_address, suggested_task_type_id, status, folder, source, message_id,
                fecha_creacion, operador_id, duracion_atencion_minutos, tiempo_espera_minutos
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDIENTE', 'INBOX', ?, ?, ?, NULL, 0, 0)
            """, (
                code,
                sender_email,
                recipient_email or "operaciones@inter.com.ve",
                subject,
                body_text,
                html_body or "",
                p.get("detected_area", "Soporte"),
                departamento_id,
                p.get("subscriber_code") or "N/A",
                p.get("serial_pon") or "N/A",
                p.get("node_name") or "N/A",
                p.get("slot_pon") or "Consultar en OLT vía Serial PON",
                p.get("mac_address") or "No provista (Usar Serial PON)",
                task_type_id,
                source,
                message_id,
                now_str
            ...
            # -------------------------------------------------------------
            # 8. REGISTRO DE EVIDENCIAS Y ARCHIVOS ADJUNTOS
            # -------------------------------------------------------------
            if attachments:
                for att in attachments:
                    cur.execute("""
            ...
            # -------------------------------------------------------------
            # 9. REGISTRO INMUTABLE EN ticket_historial_estados
            # -------------------------------------------------------------
            nota_inicial = (
                f"Ingesta técnica automática asignada a {departamento_nombre} "
                f"[{task_code} - {task_points} pts - {task_name}]"
            )
            cur.execute("""
            INSERT INTO ticket_historial_estados (
                ticket_id, operador_id, estado_anterior, estado_nuevo, nota_cambio, fecha_cambio
            ) VALUES (?, NULL, 'N/A', 'Pendiente', ?, ?)
            """, (ticket_id, nota_inicial, now_str))
            ...
            # -------------------------------------------------------------
            # 11. REGISTRO EN AUDITORÍA FORENSE GENERAL
            # -------------------------------------------------------------
            try:
                log_audit_event(
                    action="INGESTA_TICKET",
                    entity_type="TICKET",
                    entity_id=code,
                    details=f"Ticket {code} creado en {departamento_nombre} con complejidad {task_code} ({task_points} pts)",
                    ip_address="MAIL_WORKER"
                )
            except Exception:
                pass
            ...
            conn.rollback()
            logger.warning(f"sqlite3.IntegrityError al insertar ticket (deduplicación prevenida): {ie}")
            return None
        except Exception as e:
            conn.rollback()
            self.last_error = str(e)
            logger.error(f"Error inesperado al insertar ticket en SQLite: {e}")
            return None
        finally:
            conn.close()
